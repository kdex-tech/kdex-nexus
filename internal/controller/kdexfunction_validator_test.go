...
var _ = Describe("KDexFunction Validator", func() {
	Context("When creating or updating a KDexFunction", func() {
		...
		It("should fail if manual function has no language", func() {
			...
			err := k8sClient.Create(ctx, resource)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("spec.function.language is required for manual functions"))
		})

		It("should fail if manual function has no codePackage", func() {
			resource := &kdexv1alpha1.KDexFunction{
				ObjectMeta: metav1.ObjectMeta{
			...
			err := k8sClient.Create(ctx, resource)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("spec.function.codePackage is required for manual functions"))
		})

		It("should fail if state is Ready but no implementation details", func() {
			resource := &kdexv1alpha1.KDexFunction{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-ready-no-impl",
					Namespace: namespace,
				},
				Spec: kdexv1alpha1.KDexFunctionSpec{
					HostRef: corev1.LocalObjectReference{Name: "some-host"},
			...
			// Try to set state to Ready without implementation
			resource.Status.State = kdexv1alpha1.KDexFunctionStateReady
			err = k8sClient.Status().Update(ctx, resource)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("function cannot be in Ready state without a codePackage or valid SourceImage"))
		})
	})
})
