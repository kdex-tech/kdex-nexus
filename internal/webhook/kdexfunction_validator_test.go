...
func TestKDexFunctionValidator_ValidateCreate(t *testing.T) {
	...
	t.Run("manual function missing codePackage", func(t *testing.T) {
		function := &kdexv1alpha1.KDexFunction{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-function",
				Namespace: "default",
		...
		warnings, err := validator.ValidateCreate(ctx, function)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "spec.function.codePackage is required")
		assert.Empty(t, warnings)
	})

	t.Run("ready state without implementation", func(t *testing.T) {
		function := &kdexv1alpha1.KDexFunction{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-function",
				Namespace: "default",
		...
		warnings, err := validator.ValidateCreate(ctx, function)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "cannot be in Ready state without a codePackage or valid SourceImage")
		assert.Empty(t, warnings)
	})
}
